...
class MenirSynapse:
    ...
    async def _queue_command(self, raw_input: str, origin: str) -> str:
        ...
        payload: CommandPayload = await self.logos.interpret_intent(raw_input, origin)
        ...
        logger.info(f"📥 [CommandBus] {origin} enviou {payload.action} (Prioridade: {priority})")
    ...
    async def handle_command_http(self, request):
        ...
        response_str = await self._queue_command(raw_intent, origin)
