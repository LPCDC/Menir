...
class MenirProv:
    def __init__(self, ontology_manager: MenirOntologyManager):
        ...
        self.REGISTERABLE_ACTIONS = {
            "PAUSE_INGESTION", "RELOAD_RULES", "FLUSH_QUARANTINE", "RESUME_INGESTION",
            "RECONCILIATION_CRITICAL_MATCH", "DOWNSAMPLING_EXCEPTION", "GHOST_NODE_ISOLATED",
            "CRESUS_EXPORT_FORCED"
        }

    def record_event(self, agent: str, action: str, target_metadata: dict, target_node_id: str = None):
        """
        ...
        try:
            ...
            logger.critical(f"Falha de Auditoria Irreversível! O Evento {action} de {agent} não foi persistido no Grafo: {e}")
