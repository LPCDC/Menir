    ...
    def perform_sanity_check(self, tenant: str) -> dict:
        ...
        try:
            with self.ontology_manager.driver.session() as session:
                ...
                report["ghost_invoices"] = (r1["isolated"] if r1 else 0) + (r2["isolated"] if r2 else 0)
        ...
        if total_ghosts > 0:
            logger.critical(f"⚠️ ALERTA DE QUARENTENA: O Sanitizer isolou {total_ghosts} Fragmentos Fantasmas! "
            ...
            logger.info("✅ Sanity Check concluído. A integridade do Grafo está em 100%. Nenhum fantasma encontrado.")
