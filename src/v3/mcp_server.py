...
class MenirMCPServer:
            ...
            "purge_tenant_data": {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        "confirm": {
                            "type": "boolean",
                            "description": "Passar True para explodir tudo.",
                        }
                    },
                    "required": ["confirm"],
                },
    ...
    async def get_allowed_tools(self, tenant_id: str) -> list[dict]:
        ...
        """
        allowed = []
        for name, tool in self.tools_registry.items():  # noqa: B007
            if tenant_id in tool.get("allowed_tenants", []) or (
                "ROOT" in tool.get("allowed_tenants", []) and tenant_id == "ROOT"
            ):
    ...
    async def execute_tool(self, tenant_id: str, tool_name: str, arguments: dict) -> Any:
        ...
        if not tool or (tenant_id not in tool.get("allowed_tenants", []) and tenant_id != "ROOT"):
            raise Exception(
                f"Galvanic Isolation Violation: Tool '{tool_name}' not found or not accessible to Tenant '{tenant_id}'"
            )
        ...
        if tool_name == "validate_swiss_vat":
            ...
            return {
                "status": "SUCCESS",
                "message": f"CRÉSUS tabular export concluded for Tenant {tenant_id}.",
            }
    ...
    async def handle_mcp_request(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            logger.info(
                f"🛡️ [WebMCP Gateway] Inbound '{method}' request from Tenant Identity: {tenant_context}"
            )
