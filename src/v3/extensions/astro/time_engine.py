...
class MenirTime:
    ...
    def calculate_transits(
        self, natal_placements: list, target_dt: datetime, lat: float = 0, lon: float = 0
    ) -> dict[str, Any]:
        """
        ...
        forecast: dict[str, Any] = {"target_date": target_dt.isoformat(), "aspects": []}
        ...
        for transit_body_id in MAJOR_PLANETS:
            if transit_body_id not in const.LIST_OBJECTS and transit_body_id not in [
                "Chiron",
                "North Node",
                "South Node",
            ]:
...
if __name__ == "__main__":
    ...
    for a in res["aspects"]:
        print(
            f"✨ {a['transit_planet']} ({a['transit_sign']}) {a['aspect']} Natal {a['natal_planet']} (Orb: {a['orb_diff']}°)"
        )
