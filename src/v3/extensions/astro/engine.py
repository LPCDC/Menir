...
class MenirAstro:
    ...
    def calculate_chart(self, dt: datetime, lat: float, lon: float, city: str = "Unknown") -> tuple[BirthChart, list[Placement]]:
        """
        Calculates the Birth Chart.
        Returns: (BirthChart Node, List of Placement Nodes)
        """
        # 1. Convert to Flatlib Date
        date = Datetime(dt.strftime("%Y/%m/%d"), dt.strftime("%H:%M"), "+00:00") # Assuming UTC for simplicity or handle offsets
        pos = GeoPos(lat, lon)
...
if __name__ == "__main__":
    ...
    for p in planets:
        print(f"{p.properties['body_name']} in {p.properties['sign_name']} ({p.degree_in_sign:.2f})")
