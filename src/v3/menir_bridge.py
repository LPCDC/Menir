...
from src.v3.schema import BaseNode, Person, Document, Organization, Relationship
...
class MenirBridge:
    ...
    def check_document_exists(self, sha256: str, tenant_id: str = None) -> bool:
        ...
        if not tenant_id:
            logger.warning("check_document_exists called without tenant_id. This is a security risk. Denying query.")
            return False
    ...
    def merge_node(self, node: BaseNode):
        ...
        if not tenant_id:
             raise ValueError(f"Tenant_ID (project) missing for node {getattr(node, 'name', 'Unknown')}.")
        ...
        if isinstance(node, Document):
            ...
            """
             params = {"name": getattr(node, 'name', 'Unknown'), "project": node.project, "props": props}
    ...
    def link_author(self, doc_hash: str, author_name: str, tenant_id: str):
        ...
        try:
            with self.driver.session() as session:
                ...
                if result:
                    logger.info(f"Metadata Link: Linked '{safe_name}' to '{result['linked_name']}' (EXACT).")
                    return

                # 2. Try Fuzzy
                result = session.run(query_fuzzy, name=safe_name, first_name=first_name, sha=doc_hash).single()
                if result:
                    logger.info(f"Metadata Link: Linked '{safe_name}' to '{result['linked_name']}' (FUZZY).")
                else:
                    logger.info(f"Metadata Link: Author '{safe_name}' not found or ambiguous (Skipped).")
    ...
    def init_vector_index(self):
        ...
        try:
            ...
            if "already exists" in str(e).lower() or "equivalent" in str(e).lower():
                logger.info("ℹ️ Vector Index 'menir_vectors' already exists.")
            else:
    ...
    def vector_search(self, embedding: list, tenant_id: str, limit: int = 5, min_score: float = 0.7):
        """
